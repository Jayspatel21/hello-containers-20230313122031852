...
const JWT_SECRET = 'JayPatel';
const RESET_PASSWORD_KEY = 'dfhhmtnyuntmryu';
...
exports.postLogin = async (req, res, next) => {
  ...
  if (
    ...
    const token = jwt.sign({ id: users[0].admin_id }, JWT_SECRET, {
...
exports.resetPassword = (req, res, next) => {
  ...
  if (password !== confirmPass) {
    ...
    if (resetLink) {
      jwt.verify(resetLink, RESET_PASSWORD_KEY, (err, data) => {
        if (err) {
